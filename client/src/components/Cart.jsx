...
const Cart = () => {
  ...
  const handleCheckOut = async () => {
    const paymentDone = await usePayment(cost);
    console.log("Payment Done => ", paymentDone);
    if (paymentDone === true) {
      const success = await pushCartToDatabase();
      if (success) {
        alert("Payment done the flights are booked successfully!!!");
        handleClearCart();
      }
    } else {
    ...
    <div className="cart">
      ...
      {bookedItems?.length == 0 ? (
        <div className="flex-column">
          ...
          <p>You can go to flights page to view more flights</p>
        </div>
      ) : (
        bookedItems.map((item, index) => {
          return <CartItemCard key={item?.userId + item?.flightId} {...item} />;
        })
      )}
    </div>
  );
