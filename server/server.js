...
const { User, Flight, Employee, Booking } = require("./models");
...
app.post("/add-flight", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Flight added successfully", flight: savedFlight });
  } catch (error) {
    res.status(500).json({ message: "Server error" });
  }
});
...
// Api Endpoint To See All Flights
app.get("/get-all-flights", async (req, res) => {
  try {
...
// route definition for booking flights
app.post("/book-flight", async (req, res) => {
  ...
  try {
    ...
    for (const bookingData of bookings) {
      const { bookId, flightId, userId, departureDate, returnDate } =
        bookingData;
    ...
    res
      .status(500)
      .json({ message: "Server error. Failed to book the flight." });
  }
});
